...
def main():
    ...
    with open('cv_fold_indices.json', 'r+') as f:
        folds = json.load(f)

    output_fname = 'results_BETO_CV.json'
    # with open(output_fname, 'w+') as f:
    #     print("[", file=f)

    results = []
    for feat in folds:
        selection = upv[upv[feat].notnull()].copy()
        selection['label'] = selection.loc[:, feat]
        for camp in folds[feat]:
            for fold in folds[feat][camp]:
                train_indices = folds[feat][camp][fold]['train_indices']
                test_indices = folds[feat][camp][fold]['test_indices']
                data = {
                    'train': selection.loc[train_indices],
                    'test': selection.loc[test_indices]
                }
                # Output dirs
                model_path = "_"
                ...
                print(
                    f'Training {model_checkpoint} on {camp}.')
                ...
                accuracy = np.mean(preds == data['test'][feat])
                try:
                    f1 = f1_score(y_true=data['test']
                                  [feat], y_pred=preds)
                except:
                    f1 = f1_score(y_true=data['test'][feat],
                                  y_pred=preds, average='macro')

                cf = confusion_matrix(
                    y_true=data['test'][feat], y_pred=preds)

                record = {
                    'feat': feat,
                    'train': camp, 'test': camp,
    ...
    results['clf'] = 'BETO_CV'
    results = results.groupby(['feat', 'test'])[['accuracy', 'f1']].mean()
